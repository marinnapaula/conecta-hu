    ...
    if not df_inv.empty:
        ...
        with fin_g2:
            ...
            if 'AQUISIÇÃO' in df_fin_ativos.columns:
                ...
                df_invest_tempo['text_year'] = df_invest_tempo['AQUISIÇÃO'].dt.year
                df_curva = df_invest_tempo[df_invest_tempo['text_year'].notna() & (df_invest_tempo['text_year'] >= 2010)].groupby('text_year')['VALOR (R$)'].sum().reset_index(name='Investimento')
                fig_curva = px.area(df_curva, x='text_year', y='Investimento', markers=True, color_discrete_sequence=['#32A347'])
...
with tab_historico:
    ...
    # Processamento Matemático por Arquivo
    ...
    if not df_curva_fila.empty:
        ...
        df_hist_agrupado.rename(columns={'DT_SNAP': 'Data'}, inplace=True)
        ...
        if total_ativos_filtro > 0: df_hist_agrupado['Taxa_Disp'] = ((total_ativos_filtro - df_hist_agrupado['Parados']) / total_ativos_filtro) * 100
        else: df_hist_agrupado['Taxa_Disp'] = 0

    # GRÁFICO: FOTO DA DISPONIBILIDADE (Largura Total)
    with st.container(border=True):
    ...
    # --- DEFINIÇÃO DA GRELHA ORIGINAL DE 4 CONTENTORES ---
    ...
    # Contentor 2 (Linha 1 - Direita): TMA - HISTÓRICO ORIGINAL
    with r1c2:
        with st.container(border=True):
            ...
            if not df_enc.empty and 'ABERTURA' in df_enc.columns and 'ENCERRAMENTO' in df_enc.columns:
                ...
                df_tma_ano = df_tma.groupby(['Ano', 'MesNum', 'MesNome'])['DURACAO'].mean().reset_index().sort_values(['Ano', 'MesNum'])
    ...
    with r2c1:
        with st.container(border=True):
            ...
            if not df_hist_agrupado.empty and 'Tempo_Medio_Aberta' in df_hist_agrupado.columns:
    ...
    with r2c2:
        with st.container(border=True):
            ...
            if not df_hist_agrupado.empty and 'Criticas' in df_hist_agrupado.columns:
    ...
    # Contentor 5 (Largura Total): TMA X MÊS
    with st.container(border=True):
